...
class ConvertWork(QThread):
        ...
        try:
            ...
            if not label_paths:
                ...
                for idx, p in enumerate(label_paths, 1):
                    result_data, convert_ext = self._convert_dataset(
                        p, self.input_label, self.output_label
                    )
                    self._save_label(result_data, p, convert_ext)
    ...
    @staticmethod
    def _convert_dataset(label_path, input_label_type, convert_label_type):
        BACKGROUND_LABEL = 0
        ...
        convert_object = label_object.convert_dataset_by_convert_type(
            convert_object, convert_label_type
        )
        return convert_object, convert_ext

    @staticmethod
    def _save_label(result_data, label_path, convert_ext):
